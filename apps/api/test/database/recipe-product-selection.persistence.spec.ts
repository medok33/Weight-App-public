...
describe('RECIPE-PRODUCT-SELECTION-POLICY-01 disposable acceptance', () => {
  it('persists deterministic decisions twice without new rows or changed selections', async () => {
    await withDisposableMigratedDb(async ({ pool, connectionString }) => {
      ...
      expect(first.metrics.COHORT_CLUSTERS_ANALYZED).toBe(9); expect(second.metrics).toEqual(first.metrics); expect(firstRows.rows[0]).toEqual(secondRows.rows[0]); expect(firstDecisions.rows).toEqual(secondDecisions.rows); expect(first.briefsUpdated).toBe(second.briefsUpdated); expect(firstRows.rows[0]!.metadata).toBe('9');
    }, 300000);
  }, 300000);
});
