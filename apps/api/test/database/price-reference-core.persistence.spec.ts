...
describe('PRICE-01A dependable reference price persistence', () => {
  it('proves current/stale/conditional/unknown and store isolation', async () => {
    ...
    await repo.insertObservation({ productId: product.rows[0]!.id, storeId: stores[0]!.rows[0]!.id, retailerId: retailer.rows[0]!.id, externalSku: `sku_a_${suffix}`, productTitle: 'A', packageValue: 500, packageUnit: 'g', price: 100, currency: 'RUB', sourceType: 'CSV', sourceName: 'CSV Import', collectedAt: now.toISOString(), acquiredAt: now.toISOString(), acquisitionTimeQuality: 'MEASURED', legacySource: 'csv' });
    ...
    await repo.insertObservation({ productId: productB.rows[0]!.id, storeId: stores[0]!.rows[0]!.id, retailerId: retailer.rows[0]!.id, externalSku: `sku_b_${suffix}`, productTitle: 'B', packageValue: 1, packageUnit: 'piece', price: 50, currency: 'RUB', sourceType: 'CSV', sourceName: 'CSV Import', collectedAt: old, acquiredAt: old, acquisitionTimeQuality: 'MEASURED', legacySource: 'csv' });
    ...
    await repo.insertObservation({ productId: productC.rows[0]!.id, storeId: stores[0]!.rows[0]!.id, retailerId: retailer.rows[0]!.id, externalSku: `sku_c_${suffix}`, productTitle: 'C', packageValue: 500, packageUnit: 'g', price: 80, currency: 'RUB', sourceType: 'CSV', sourceName: 'CSV Import', collectedAt: now.toISOString(), acquiredAt: now.toISOString(), acquisitionTimeQuality: 'MEASURED', legacySource: 'csv', priceCondition: 'LOYALTY_ONLY' });
    ...
    await repo.insertObservation({ productId: productD.rows[0]!.id, storeId: stores[1]!.rows[0]!.id, retailerId: retailer.rows[0]!.id, externalSku: `sku_d_${suffix}`, productTitle: 'D', packageValue: 500, packageUnit: 'g', price: 200, currency: 'RUB', sourceType: 'CSV', sourceName: 'CSV Import', collectedAt: now.toISOString(), acquiredAt: now.toISOString(), acquisitionTimeQuality: 'MEASURED', legacySource: 'csv' });
    ...
    await repo.insertObservation({ productId: productMapped.rows[0]!.id, storeId: stores[0]!.rows[0]!.id, retailerId: retailer.rows[0]!.id, retailProductId: unmapped.rows[0]!.id, price: 70, currency: 'RUB', sourceType: 'CSV', sourceName: 'CSV Import', collectedAt: now.toISOString(), acquiredAt: now.toISOString(), acquisitionTimeQuality: 'MEASURED', legacySource: 'csv' });
    ...
    await repo.insertObservation({ productId: productAmbiguous.rows[0]!.id, storeId: stores[0]!.rows[0]!.id, retailerId: retailer.rows[0]!.id, retailProductId: ambiguous.rows[0]!.id, price: 60, currency: 'RUB', sourceType: 'CSV', sourceName: 'CSV Import', collectedAt: now.toISOString(), acquiredAt: now.toISOString(), acquisitionTimeQuality: 'MEASURED', legacySource: 'csv' });
    ...
    await repo.insertObservation({ productId: productPromo.rows[0]!.id, storeId: stores[0]!.rows[0]!.id, retailerId: retailer.rows[0]!.id, externalSku: `promo_a_${suffix}`, productTitle: 'Promo', packageValue: 500, packageUnit: 'g', price: 40, currency: 'RUB', sourceType: 'CSV', sourceName: 'CSV Import', collectedAt: now.toISOString(), acquiredAt: now.toISOString(), acquisitionTimeQuality: 'MEASURED', legacySource: 'csv', priceCondition: 'LOYALTY_ONLY' });
    await repo.insertObservation({ productId: productPromo.rows[0]!.id, storeId: stores[1]!.rows[0]!.id, retailerId: retailer.rows[0]!.id, externalSku: `promo_b_${suffix}`, productTitle: 'Promo', packageValue: 500, packageUnit: 'g', price: 90, currency: 'RUB', sourceType: 'CSV', sourceName: 'CSV Import', collectedAt: now.toISOString(), acquiredAt: now.toISOString(), acquisitionTimeQuality: 'MEASURED', legacySource: 'csv', priceCondition: 'REGULAR' });
