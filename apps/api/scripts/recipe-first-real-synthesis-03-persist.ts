...
import { RecipeQualityOrchestrator } from '../src/modules/recipe-platform/application/recipe-quality.orchestrator.ts';
import { validateCulinaryCriticResult } from '../src/modules/recipe-platform/domain/culinary-critic.policy.ts';
...
const artifact = JSON.parse(readFileSync(artifactPath, 'utf8')) as { editor: { steps: Array<{ stepId: string; text: string }> }; critic?: { verdict: string; issues: Array<{ code: string; message?: string }> } };
...
async function main() {
  ...
  const quality = await new RecipeQualityOrchestrator().verify({
    ...
    base: { contractVersion: 1, recipeKey: FIRST_REAL_SYNTHESIS_RECIPE_KEY, versionIdentity: 'recipe-first-real-synthesis/v1', title: 'Жульен с курицей и грибами', description: 'Синтезированный рецепт классического жульена.', servings: 4, yieldGrams: 1427.3, ingredients: ingredients.map((item) => ({ ingredientId: item.id, productId: item.productId, grams: item.amount, unit: item.unit, optional: false })), equipment: ['PAN', 'OVEN', 'BAKING_DISH', 'GRATER'], methodSkeleton: skeleton, nutrition, cost: { status: 'UNAVAILABLE', currency: 'RUB' }, safety: { status: 'PASS', reasons: [] }, provenance: { sourceIds: ['research-cluster:dcluster_8c521f996b1e8844f530ff12'], evidenceIds: ['structured-facts-only'] }, similarity: { autoPublish: true, decision: 'CREATE', score: 0.45 }, cookTestStatus: 'NOT_PERFORMED', publicationState: 'DRAFT' },
    editor: async () => ({ title: 'Жульен с курицей и грибами', description: 'Синтезированный рецепт классического жульена.', steps: artifact.editor.steps }),
    critic: async () => artifact.critic ? validateCulinaryCriticResult(artifact.critic) : { contractVersion: 'culinary-critic/v1', verdict: 'REJECT', issues: [] },
    semanticCoverage: { requiredTerms: ['курин', 'шампин', 'сметан', 'сыр', 'оливков'], forbiddenTerms: /рис|майонез/ },
  });
  if (quality.status !== 'AUTO_VERIFIED' || !quality.contract || !quality.receipt) throw new Error(`QUALITY_GATE_FAILED:${quality.reasons.join(',')}`);
  const service = new RecipePublicationService(db);
  const input = { recipeKey: FIRST_REAL_SYNTHESIS_RECIPE_KEY, title: 'Жульен с курицей и грибами', description: 'Синтезированный рецепт классического жульена.', servings: 4, yieldGrams: 1427.3, ingredients, steps, nutrition, cost: { status: 'UNAVAILABLE', currency: 'RUB' }, similarityAutoPublish: true, actorId: '00000000-0000-0000-0000-000000000001', provenance: { synthesisBriefId: 'recipe-first-real-synthesis/v1', clusterId: 'dcluster_8c521f996b1e8844f530ff12', nutritionAuthority: 'ProductNutritionVersion', editorArtifact: 'recipe-first-real-synthesis-attempt-2' }, qualityContract: quality.contract, qualityReceipt: quality.receipt };
