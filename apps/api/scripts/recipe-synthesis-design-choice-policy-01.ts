...
export async function runDesignChoiceAudit(): Promise<{ result: Awaited<ReturnType<typeof runProductSelection>>; audited: PendingAuditRow[] }> {
  ...
  if (result.metrics.COHORT_CLUSTERS_ANALYZED !== 9 || result.metrics.TOTAL_REQUIRED_INGREDIENTS !== 178 || result.metrics.PRODUCT_CATALOG_GAP !== 13 || result.metrics.PRODUCT_SELECTION_PENDING !== 24 || result.metrics.READY_FOR_DETERMINISTIC_GRAMS_AFTER !== 3) {
  ...
  if (audited.length !== 24) throw new Error(`PENDING_AUDIT_COUNT:${audited.length}`);
