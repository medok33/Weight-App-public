...
export function runPipeline(): PipelineResult {
  ...
  for (const group of grouped.values()) { const cluster = buildDishConceptCluster(group, '2026-08-20T00:00:00.000Z'); const effectiveGroup = group.map((candidate) => applyBoundedContext(candidate, accepted)); clusters.push(cluster); const facts = aggregateResearchFacts(cluster, effectiveGroup, '2026-08-20T00:00:00.000Z'); allFacts.push(...facts); const required = effectiveGroup.flatMap((c) => c.ingredients.filter((i) => i.role === 'REQUIRED')); const exact = required.filter((i) => i.productId && !String(i.productId).startsWith('family:')).length; const familyPending = required.filter((i) => String(i.productId ?? '').startsWith('family:')).length; const gaps = required.filter((i) => !i.productId).length; const conflicts = facts.filter((f) => f.requiresReview).length; const highConflicts = facts.filter((f) => f.conflictLevel === 'HIGH').length; const multiSource = cluster.sourceCount >= 2; const eligible = multiSource && gaps === 0 && highConflicts === 0 && effectiveGroup.every((c) => c.ingredients.every((i) => i.productId)); const cohortSelected = firstCohortGroups.has(group); let brief: SynthesisBrief | null = null; if (eligible || cohortSelected) { brief = buildSynthesisBrief({ cluster, facts, objective: 'first-real-corpus-readiness', coverageSlot: 'RESEARCH', approvedProducts: [...new Set(required.map((i) => i.productId).filter((id): id is string => Boolean(id) && !id.startsWith('family:')))], allowedEquipment: [...new Set(effectiveGroup.flatMap((c) => c.equipment ?? []))] }); if (gaps > 0) { brief.unresolvedFacts = [...brief.unresolvedFacts, `PRODUCT_IDENTITY_PENDING:${required.filter((i) => !i.productId).map((i) => i.name).join('|')}`]; if (brief.status !== 'BLOCKED_CONFLICT') brief.status = 'READY_FOR_REVIEW'; } briefs.push(brief); }
