...
export function evaluateCulinarySafety(input: { category: 'poultry'|'meat'|'fish'|'egg'|'reheating'|'none'; steps: AuthoringStep[]; storageInstructions?: string }): { status: 'PASS'|'NEEDS_SAFETY_REVIEW'|'FAIL'; reasons: string[] } { if (input.category === 'none') return { status: 'PASS', reasons: [] }; const text = input.steps.map((s) => s.text).join(' ').toLowerCase(); const hasHeat = input.steps.some((s) => (s.temperatureC ?? 0) >= 63) || /варить|жарить|запек|готовить|нагрев|кипят/.test(text); if (!hasHeat) return { status: 'FAIL', reasons: ['REQUIRED_HEAT_HANDLING_MISSING'] }; if (input.category === 'reheating' && /хранить.*комнат|оставить.*тепл/.test(`${text} ${input.storageInstructions ?? ''}`)) return { status: 'FAIL', reasons: ['REHEAT_STORAGE_CONTRADICTION'] }; return { status: 'PASS', reasons: [] }; }
