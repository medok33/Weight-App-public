/** STEP_215C — RussianFood fixture-backed adapter. Live HTTP blocked by transport. */
...
import {
  ...
  buildRussianFoodSearchUrl,
  RUSSIANFOOD_HOSTNAME_ALLOWLIST,
...
} from './russianfood.parser';

export class RussianFoodSourceAdapter implements RecipeSourceAdapter {
  readonly adapterType = 'RUSSIANFOOD';
  readonly contractVersion = RECIPE_SOURCE_ADAPTER_CONTRACT_VERSION;
  readonly parserVersion = RUSSIANFOOD_PARSER_VERSION;
  readonly descriptor: RecipeSourceAdapterDescriptor = {
    ...
    supportedLocales: ['ru'],
    supportedSourceCodes: [RUSSIANFOOD_SOURCE_CODE],
  };
  ...
  constructor(input?: {
    ...
    this.fixtureTransport =
      ...
      createFixtureTransport((scenario) => resolveRussianFoodFixture(scenario));
  }
  ...
  async searchByProducts(
    ...
    const url = buildRussianFoodSearchUrl(String(query).slice(0, 80));
  ...
  async fetchCandidate(
    ...
    const url = buildRussianFoodRecipeUrl(urlSlugForScenario(scenario, 'synthetic-chicken-buckwheat'));
    const transport = this.pickTransport(context);
    try {
      const response = await transport.request({
        sourceCode: RUSSIANFOOD_SOURCE_CODE,
  ...
  async healthCheck(context: RecipeSourceExecutionContext): Promise<SourceAdapterHealthResult> {
    ...
    if (!context.testMode) {
      return {
        ...
        details: 'Live RussianFood execution is policy-blocked; fixture mode required',
        checkedAt: new Date().toISOString(),
      };
    }
  ...
  private assertContext(context: RecipeSourceExecutionContext, operation: string): void {
    ...
    const allowedCode = code === RUSSIANFOOD_SOURCE_CODE || code.startsWith('russianfood_');
    if (!allowedCode || (adapterType !== 'RUSSIANFOOD' && adapterType !== 'NOT_CONFIGURED')) {
      throw new RecipeSourceAdapterError({
        code: 'CONFIGURATION_ERROR',
        sourceCode: context.sourceCode,
        operation,
        retryable: false,
        safeMessage: 'RussianFood adapter sourceCode/adapterType mismatch',
        correlationId: context.correlationId,
        parserVersion: this.parserVersion,
      });
    }
