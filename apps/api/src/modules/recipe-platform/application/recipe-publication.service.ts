...
export class RecipePublicationService {
  ...
  async publish(input: PublicationInput) {
    ...
    return this.db.withTransaction(async (query) => {
      ...
      const version = await query<{ id: string }>(`INSERT INTO "RecipeVersion" ("recipeId","versionNumber",status,"contentSnapshotJson","ingredientsSnapshotJson","stepsSnapshotJson","nutritionSnapshotJson","costSnapshotJson","restrictionSnapshotJson","servings","servingWeightGrams","changeType","createdBy","approvedBy","approvedAt","publishedAt",checksum,provenance) VALUES ($1,$2,'PUBLISHED',$3::jsonb,$4::jsonb,$5::jsonb,$6::jsonb,$7::jsonb,'{}'::jsonb,$8,$9,$10,$11,$11,now(),now(),$12,'SYSTEM') RETURNING id`, [recipeId, versionNumber, JSON.stringify({ title: input.title, description: input.description, servings: input.servings, automatedQualityPass: input.automatedQualityPass === true, cookTestStatus: input.cookTest ? (input.cookTest.decision === 'PASS' ? 'PASSED' : 'FAILED') : 'NOT_PERFORMED' }), JSON.stringify(input.ingredients), JSON.stringify(input.steps), JSON.stringify(input.nutrition), JSON.stringify(input.cost), input.servings, input.yieldGrams / input.servings, input.automatedQualityPass === true ? 'AUTO_PUBLISH' : 'MANUAL_PUBLISH', input.actorId, checksum]);
