...
export type PublicationInput = { recipeKey: string; title: string; description: string; servings: number; yieldGrams: number; ingredients: AuthoringIngredient[]; steps: AuthoringStep[]; nutrition: RecipeNutrition; cost: unknown; validationPass: boolean; similarityAutoPublish?: boolean; editorial: HumanReview; cookTest: CookTest; actorId: string };

@Injectable()
export class RecipePublicationService {
  constructor(@Inject(PrismaService) private readonly db: PrismaService) {}
  async publish(input: PublicationInput) {
    const gate = canPublish({ editorial: input.editorial, cookTest: input.cookTest, validationPass: input.validationPass, costStatus: (input.cost as { status?: 'PASS'|'UNAVAILABLE' })?.status ?? 'UNAVAILABLE', similarityAutoPublish: input.similarityAutoPublish });
    ...
    return this.db.withTransaction(async (query) => {
      ...
      const version = await query<{ id: string }>(`INSERT INTO "RecipeVersion" ("recipeId","versionNumber",status,"contentSnapshotJson","ingredientsSnapshotJson","stepsSnapshotJson","nutritionSnapshotJson","costSnapshotJson","restrictionSnapshotJson","servings","servingWeightGrams","changeType","createdBy","approvedBy","approvedAt","publishedAt",checksum,provenance) VALUES ($1,$2,'PUBLISHED',$3::jsonb,$4::jsonb,$5::jsonb,$6::jsonb,$7::jsonb,'{}'::jsonb,$8,$9,'MANUAL_PUBLISH',$10,$10,now(),now(),$11,'SYSTEM') RETURNING id`, [recipeId, versionNumber, JSON.stringify({ title: input.title, description: input.description, servings: input.servings }), JSON.stringify(input.ingredients), JSON.stringify(input.steps), JSON.stringify(input.nutrition), JSON.stringify(input.cost), input.servings, input.yieldGrams / input.servings, input.actorId, checksum]);
