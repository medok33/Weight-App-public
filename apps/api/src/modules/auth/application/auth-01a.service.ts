...
export class Auth01aService {
  constructor(@Inject(PrismaService) private readonly db: PrismaService, @Inject(AuthService) private readonly auth: AuthService, @Inject(AuthRepository) private readonly repository: AuthRepository, @Inject(TestAuthDelivery) private readonly delivery: TestAuthDelivery, @Inject(RedisAuthAbuseService) private readonly redisAbuse: RedisAuthAbuseService) {}
  ...
  async register(emailInput: unknown, password: unknown, inviteInput?: unknown, ip?: string, anonymousUserId?: string) {
    ...
    const accountHash = authAbuseHash(`account:${email}`);
    await this.redisAbuse.assertAvailableFor('register', accountHash);
    const registrationBlock = await this.repository.recordRegistrationAttempt({ accountHash, ipHash: authAbuseHash(`ip:${normalizeClientAddress(ip)}`) });
