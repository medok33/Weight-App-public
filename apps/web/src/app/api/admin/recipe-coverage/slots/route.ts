...
export async function GET(request: Request) {
  ...
  return proxy(request, `/admin/recipe-coverage/slots?${url.searchParams.toString()}`);
}

export async function POST(request: Request) {
  return proxy(request, '/admin/recipe-coverage/slots', { method: 'POST', body: await request.text() });
}
