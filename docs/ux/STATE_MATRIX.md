...
| Basket | `SUCCESS`; all current basket lines have applicable evidence | `BasketPurchaseEstimate` is the primary estimated payable amount. A reliable whole-basket total is shown only when every required line meets the domain reliability policy; `PlanReferenceCostEstimate` is not shown as Basket payable amount. Show package/purchase quantity, store, substitution and override. | Preview/update / remove or change | `MATCH_CONFIDENCE`, `PRICE_CONFIDENCE` and `PRICE_FRESHNESS` are separately labelled; reference never looks live or guaranteed checkout |
| Basket | `PARTIAL_DATA`, `PRICE_DATA_INCOMPLETE`; one or more required lines lack usable monetary evidence | Explicit incomplete-price state; known-lines amount may be labelled partial/estimated, or the whole-basket estimate is unavailable | Refresh/recalculate / substitute, remove, list | A partial sum never appears as complete total or budget conclusion; package-match uncertainty remains separate |
| Basket | `STALE`, `OFFLINE`; monetary evidence is dated/unrefreshable | Dated Basket estimate only if owner permits, with freshness and price-confidence state | Refresh when available / retain list, substitute or return | Staleness does not change match confidence; no current/live/complete-total claim without full suitable coverage |
