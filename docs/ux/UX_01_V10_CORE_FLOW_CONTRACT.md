...
| Basket | A future Basket is a purchase-estimate review, not checkout. Entry requires a selected shopping item or basket snapshot. Each item shows package/purchase quantity, store scope, match confidence, reference/confirmed amount only with provenance, substitution and user removal/override. Empty, partial, stale, offline, error and recalculating states keep uncertainty visible. Missing package match or price produces uncertainty; no reliable amount means no exact total. Whole-basket delta and `OVER_BUDGET_PREVIEW` are preview states, not payable facts. Primary recovery is refresh/recalculate when data permits; otherwise return to list or change a non-hard preference. |
